...
def get_supported_countries_from_config(config: RAGConfig) -> Set[str]:
    """
    Extract supported countries from json_folders paths.
    Expected folder structure: <base>/<Country>
    Example: Contest_Data/Italy -> Italy
    """
...
# =====================================================================
# Dataset Registry (kept simple, NOT authoritative)
# =====================================================================

def build_db_registry(config: RAGConfig) -> List[Dict]:
    """
    Registry is NOT used for country validation.
    It is only used to know which vector DBs exist.
    """
    registry: List[Dict] = []
...
# =====================================================================
# Semantic selection (ONLY country & type)
# =====================================================================
...
def country_gate(
    ...
    """
    Retrieval is allowed if at least one supported country
    (derived from json_folders) is present.
    """
    ...
    if not supported_countries.intersection(countries):
        return (
            False,
            "❌ Retrieval blocked: unsupported country. "
            f"Supported countries are {sorted(supported_countries)}.",
        )
...
# =====================================================================
# Retrieval (NO country logic here)
# =====================================================================
...
def single_agent_answer_question(
    ...
    # ---- Action ----
    if need_retrieval:
        countries, content_types, semantic_log = decide_relevant_slices(
            question, llm
        )
    ...
    if show_reasoning:
        supported_countries = sorted(
            get_supported_countries_from_config(config)
        )
